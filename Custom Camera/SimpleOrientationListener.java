...
public abstract class SimpleOrientationListener extends OrientationEventListener {
    ...
    public void onOrientationChanged(final int orientation) {
        ...
        if (orientation >= 330 || orientation < 30) {
            Stash.put(Constants.ROTATION, 0);
            Log.d("CHECK123", "onOrientationChanged   ROTATION_0");
            currentOrientation = Surface.ROTATION_0;
        } else if (orientation >= 60 && orientation < 120) {
            Stash.put(Constants.ROTATION, 90);
            Log.d("CHECK123", "onOrientationChanged   ROTATION_90");
            currentOrientation = Surface.ROTATION_90;
        } else if (orientation >= 150 && orientation < 210) {
            Stash.put(Constants.ROTATION, 180);
            Log.d("CHECK123", "onOrientationChanged   ROTATION_180");
            currentOrientation = Surface.ROTATION_180;
        } else if (orientation >= 240 && orientation < 300) {
            Stash.put(Constants.ROTATION, 270);
            Log.d("CHECK123", "onOrientationChanged   ROTATION_270");
